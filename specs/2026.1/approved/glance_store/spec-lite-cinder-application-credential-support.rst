...
:problem: The Cinder backend driver in glance_store currently uses V3Password
          ...
          using Cinder as the image backend.

          When the service user password is rotated during ZDPR, Cinder backend
...
:solution: Add application credential support to the Cinder backend driver by:
           ...
           2. Modifying the Cinder driver's authentication logic to:
           ...
           3. Update get_cinder_session() function to support application
              credentials for Cinder volume operations.

           This unified approach allows Cinder backend to use the same AC
...
:how:
      ...
      - Add application_credential_id and application_credential_secret options
        to Cinder driver configuration options
      - Check for AC credentials and use V3ApplicationCredential when
        available
